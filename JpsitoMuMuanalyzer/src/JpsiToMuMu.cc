...
// 
/**\class JpsiToMuMu JpsiToMuMu.cc JpsiToMuMu/JpsiToMuMuanalyzer/src/JpsiToMuMu.cc
...
//         Edited:   Sun, 14 January 2018 13:50 GMT
//


// system include files
#include "/afs/cern.ch/work/c/ckar/jpsimumuanalyzer/CMSSW_8_0_24/src/JpsiToMuMu/JpsitoMuMuanalyzer/plugins/JpsiToMuMu.h"

JpsiToMuMu::JpsiToMuMu(const edm::ParameterSet& iConfig):

  OutputFileName_(iConfig.getParameter<string>("OutputFileName")),
  BuildJpsiToMuMu_(iConfig.getUntrackedParameter<bool>("BuildJpsiToMuMu")),
...
void
JpsiToMuMu::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  //using namespace edm;
   clearVariables();
   ...
   cout<< "run "<<run<<" event "<<event<<" lumi "<<lumiblock<<endl;
   hltReport(iEvent);
...
void JpsiToMuMu::beginRun(edm::Run const&, edm::EventSetup const&)
{
}

// method called when ending the processing of a run  ------------
void JpsiToMuMu::endRun(edm::Run const&, edm::EventSetup const&)
{
}
void JpsiToMuMu::beginLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
{
}

// ------------ method called when ending the processing of a luminosity block  ------------
void JpsiToMuMu::endLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
{
}
// ------------ method fills 'descriptions' with the allowed parameters for the module  ------------
void JpsiToMuMu::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
void JpsiToMuMu::clearVariables(){
  run = 0;
  event = 0;
  lumiblock = 0;
  nprivtx = 0;
...
bool JpsiToMuMu::hasBeamSpot(const edm::Event& iEvent){
...
bool JpsiToMuMu::hasPrimaryVertex(const edm::Event& iEvent){
...
bool JpsiToMuMu::buildJpsiToMuMu(const edm::Event& iEvent){
  ...
  for (vector<pat::Muon>::const_iterator iMuonM = patMuonHandle->begin();
    ...
    for (vector<pat::Muon>::const_iterator iMuonP = patMuonHandle->begin();
	 iMuonP != patMuonHandle->end(); iMuonP++){
      ...
      Jpsi4V=mu14V+mu24V;
      if(Jpsi4V.M()<2.9 || Jpsi4V.M()>3.3) continue;
...
void JpsiToMuMu::calLS (double Vx, double Vy, double Vz,
...
void JpsiToMuMu::calCosAlpha(double Vx, double Vy, double Vz,
                                     double Wx, double Wy, double Wz,
                                     double VxErr2, double VyErr2, double VzErr2,
                                     double VxyCov, double VxzCov, double VyzCov,
                                     double WxErr2, double WyErr2, double WzErr2,
...
bool  JpsiToMuMu::hasGoodMuonDcaBs (const reco::TransientTrack muTrackTT,double &muDcaBs, double &muDcaBsErr)
{
...
bool JpsiToMuMu::hasGoodJpsiVertex(const reco::TransientTrack mu1TT,
...
void
JpsiToMuMu::saveJpsiVertex(RefCountedKinematicTree vertexFitTree){
  vertexFitTree->movePointerToTheTop();
  RefCountedKinematicVertex jpsi_KV = vertexFitTree->currentDecayVertex();
...
void 
JpsiToMuMu::saveJpsiCosAlpha(RefCountedKinematicTree vertexFitTree)
{
...
void 
JpsiToMuMu::saveJpsiLsig(RefCountedKinematicTree vertexFitTree)
{
  vertexFitTree->movePointerToTheTop();
  RefCountedKinematicVertex jpsi_KV = vertexFitTree->currentDecayVertex();
  double LSBS, LSBSErr;
...
void
JpsiToMuMu::calCtau(RefCountedKinematicTree vertexFitTree,
...
double
JpsiToMuMu::calEta (double Px, double Py, double Pz)
{
...
double
JpsiToMuMu::calPhi (double Px, double Py, double Pz)
{
...
void
JpsiToMuMu::saveJpsiCtau(RefCountedKinematicTree vertexFitTree)
{
...
void
JpsiToMuMu::saveSoftMuonVariables(pat::Muon iMuonM, pat::Muon iMuonP,
...
void
JpsiToMuMu::saveDimuVariables(double DCAmumBS, double DCAmumBSErr,
